...
public class ParkStandardService implements IParkStandardService {
     ...
     */
    @Override
    public BigDecimal fee(Integer marketId, String in, Integer parkTime) {
        // 初始化缴费金额
        BigDecimal countPrice = new BigDecimal(0);
        try {
            if (marketId == null || in == null || parkTime == null) return countPrice;
            ...
            if (rule == null || ruleDetails == null || parkTime <= 0) return countPrice;
            int realMinute = parkTime * 60;    //固定时长

            //跨整天
            if (realMinute % 1440 == 0) {
                log.info("跨整天");
                return rule.getDayCost().multiply(new BigDecimal(realMinute / 1440));
            } else {  //跨非整天
                int day = realMinute / 1440;
                realMinute = realMinute % 1440;
                countPrice = countPrice.add(rule.getDayCost().multiply(new BigDecimal(day)));
                log.info("跨非整天 day:{} realMinute:{} countPrice:{} ", day, realMinute, countPrice);
            }

            //0:分时计费 1：分时分段计费
            if (rule.getType() == 0) {
                RuleDetail ruleDetail = ruleDetails.get(0);
                BigDecimal money = timeSharing(realMinute, ruleDetail.getUnitType(), ruleDetail.getTimeInterval(), ruleDetail.getUnitPrice());
                ...
                DateTime endTime = startTime.plusMinutes(realMinute);
                log.info("免费时长：{} 固定时长：{}", rule.getFreeTime(), realMinute);
                log.info("入场时间：{}，开始时间：{}，结束时间：{}", inDateTime, startTime, endTime);

                //如果时段跨0点进行拆分
                CopyOnWriteArrayList<RuleDetail> resultRuleDetails = new CopyOnWriteArrayList<>();
                for (RuleDetail ruleDetail : ruleDetails) {
                    if(ruleDetail.getIntervalStart() > ruleDetail.getIntervalEnd()) {
                        RuleDetail build = RuleDetail.builder().build();
                        BeanUtils.copyProperties(ruleDetail, build);
                        resultRuleDetails.add(build.setIntervalStart(0).setIntervalEnd(ruleDetail.getIntervalEnd()));
                        resultRuleDetails.add(ruleDetail.setIntervalEnd(23));
                    }else {
                        resultRuleDetails.add(ruleDetail);
                    }
                }
                ...
                for (RuleDetail ruleDetail : resultRuleDetails) {
                ...
                //跨段
                for (RuleDetail result : resultRuleDetails) {
    ...
    /**
     * 根据传入的时间获取, 时段开始时间和结束时间
     *
     * @param startTime     开始时间
     * @param intervalStart 时段开始时间
     * @param intervalEnd   时段结束时间
     * @param flag          true：时段开始时间  false：时段结束时间
     * @return
     */
    private DateTime getDataStartOrEnd(DateTime startTime, Integer intervalStart, Integer intervalEnd, Boolean flag) {
        String s = startTime.toString("yyyy-MM-dd ") + "00:00:00";
        DateTime parse = DateTime.parse(s, DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss"));
        if (flag) {
            return parse.plusHours(intervalStart);
        } else {
            return intervalStart > intervalEnd ? parse.plusHours(intervalEnd).plusDays(1) : parse.plusHours(intervalEnd);
        }
    }
}